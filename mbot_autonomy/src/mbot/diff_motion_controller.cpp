...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            float speedSet = 0.8; //slow speed - 0.3
            // float speedSet = 4.0;

            if (cmd.vx > speedSet) cmd.vx = speedSet;
            else if (cmd.vx < -speedSet) cmd.vx = -speedSet;

            // Angular vel
            float max_ang_vel = M_PI / 8.0; //slow speed
            // float max_ang_vel = M_PI/ 2.0;
            if (cmd.wz > max_ang_vel) cmd.wz = max_ang_vel;
            else if (cmd.wz < -max_ang_vel) cmd.wz = -max_ang_vel;

            // printf("%f\t%f\n", cmd.vx, cmd.wz);
            // printf("inside diff_motion_controller");
