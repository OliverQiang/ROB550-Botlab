...
public:
    ...
    UniqueSlamPtr get_reset_slam_ptr(lcm::LCM& lcmConnection, const mbot_lcm_msgs::pose2D_t& pose = {0, 0, 0, 0})
    {
        ...
        {
            mappingOnly = localizationOnly = false;
            actionOnly = true;
        }
        else
        {
...
int main(int argc, char** argv)
{
    ...
    getopt_add_int(gopt, '\0', kNumParticlesArg, "500", "Number of particles to use in the particle filter");
    getopt_add_int(gopt, '\0', kHitOddsArg, "5", "Amount to increase log-odds when a cell is hit by a laser ray");
