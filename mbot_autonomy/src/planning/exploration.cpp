...
const float kReachedPositionThreshold = 0.03f;  // must get within this distance of a position for it to be explored
const float kReachedAngleThreshold = 0.15f;
const float sleeptime_unit = 0.75;
...
bool are_equal(const pose_vec_t& lhs, const pose_vec_t& rhs)
{
    ...
    int n = lhs.size();
    for(int i = 0; i < n; i++){
        if (!are_equal(lhs.at(i), rhs.at(i))){
            return false;
        }
    }
...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    frontier_processing_t fp = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
    currentPath_ = fp.path_selected;
    mbot_lcm_msgs::path2D_t short_path = shortenPath(currentPath_);
    lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &short_path);

    // Instead of hardcoding the sleep time, calculate time according to number of nodes in path;
    double sleeptime_explore = sleeptime_unit * short_path.path.size();
    printf("The current path will take %f seconds to exectute\n", sleeptime_explore);
    sleep(sleeptime_explore);
    ...
    status.state = mbot_lcm_msgs::exploration_status_t::STATE_EXPLORING_MAP;
     // If all frontiers have been explored as determined by find_map_frontiers()
    // if(frontiers_.empty() == true)
    if (frontiers_.size() == fp.num_unreachable_frontiers)
    {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
    }
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    if((currentPose_.x != homePose_.x) || (currentPose_.y != homePose_.y)){
        
        mbot_lcm_msgs::path2D_t path;
        path = planner_.planPath(currentPose_, homePose_);
        path.path.at(path.path.size()-1).theta = 0.0;
        // mbot_lcm_msgs::path2D_t short_path = shortenPath(path);
        lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &path);

        double sleeptime_returnHome = sleeptime_unit * path.path.size();
        printf("The returning home path will take %f seconds to exectute\n", sleeptime_returnHome);
        sleep(sleeptime_returnHome);
    }

    double distToHome = 0;
    double angleToHome = 0; 

    // Do recorrections if not returning home precisely
    for (int i = 0; i < 0; i++)
    {   
        currentPose_ = incomingPose_;
        distToHome = distance_between_points(Point<float>(homePose_.x, homePose_.y), Point<float>(currentPose_.x, currentPose_.y));
        angleToHome = std::fabs(homePose_.theta - currentPose_.theta);
        printf("Distance to home: %f, Angle to home: %f\n", distToHome, angleToHome);
        printf("Currentpose: %f  %f  %f\n",currentPose_.x, currentPose_.y, currentPose_.theta);
        printf("Homepose: %f  %f  %f\n", homePose_.x, homePose_.y, homePose_.theta);
        
        if ((distToHome > kReachedPositionThreshold) || (angleToHome > kReachedAngleThreshold)){
            mbot_lcm_msgs::path2D_t path;
            path = planner_.planPath(currentPose_, homePose_);
            printf("Returning Again...\n");
            lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &path);
            sleep(5);
        }
    }
...
mbot_lcm_msgs::path2D_t Exploration::shortenPath(mbot_lcm_msgs::path2D_t& inputPath)
{
    mbot_lcm_msgs::path2D_t trim_path;
    mbot_lcm_msgs::path2D_t short_path;
    // remove just the last waypoint
    for(int i = 0; i < inputPath.path_length - 1; i++){
        trim_path.path.push_back(inputPath.path[i]);
    }
    trim_path.path_length = trim_path.path.size();

    // to shorten the path
    int max_path_length = 30;
    if (trim_path.path_length > max_path_length){
        for(int i = 0; i < max_path_length; i++){
            short_path.path.push_back(trim_path.path[i]);
        }
        short_path.path_length = short_path.path.size();
    }
    else{
        short_path = trim_path;
    }
    return short_path;
}