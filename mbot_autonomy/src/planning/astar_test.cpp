...
mbot_lcm_msgs::path2D_t timed_find_path(const mbot_lcm_msgs::pose2D_t& start,
                             const mbot_lcm_msgs::pose2D_t& end,
                             const MotionPlanner& planner,
                             const std::string& testName);

bool is_valid_path(const mbot_lcm_msgs::path2D_t& path, double robotRadius, const OccupancyGrid& map);
bool is_safe_cell(int x, int y, double robotRadius, const OccupancyGrid& map);

std::ostream& operator<<(std::ostream& out, const mbot_lcm_msgs::pose2D_t& pose);
...
bool test_saved_poses(const std::string& mapFile, const std::string& posesFile, const std::string& testName)
{
    ...
    for(int n = 0; n < numGoals; ++n)
    {
        ...
        mbot_lcm_msgs::path2D_t path = timed_find_path(start, goal, planner, testName);
...
bool is_valid_path(const mbot_lcm_msgs::path2D_t& path, double robotRadius, const OccupancyGrid& map)
{
    ...
    mbot_lcm_msgs::path2D_t printpath;
