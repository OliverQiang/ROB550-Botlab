...
public:
    ...
    */
    float likelihood(const mbot_lcm_msgs::particle_t& particle,
                      const mbot_lcm_msgs::lidar_t& scan,
                      const OccupancyGrid& map);

    float max_scan_score;  // TODO: make getter
    void setHitCoefficient(float z_hit);
    void setRandomCoefficient(float z_rand);
...
private:
    ...
    float NormalPdf(const double& x);
    float scoreRay(const adjusted_ray_t& ray, const OccupancyGrid& map);
