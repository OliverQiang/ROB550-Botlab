...
int mbot_servo_init_freq(uint8_t ch, uint16_t freq) {
    ...
    // hard set initialization values
    if(ch == 2){
        slice = 6;
        pwm_set_chan_level(slice,0,6209);
        printf("Servo channel 2 initialized!\n");
    }
    else if(ch == 3){
        slice = 2;
        pwm_set_chan_level(slice,0,13398);
        printf("Servo channel 3 initialized!\n");
    }
    else{
        pwm_set_chan_level(slice, servo_ch[ch], 0);
        printf("random channel initialized\n");
    }
...
int mbot_servo_init(uint8_t ch) {
    // for two servo configuraiton, ensure servos are at the same angle even on reinitalization
    if (ch == 2){
...
int mbot_servo_send_pulse_us(uint8_t ch, uint32_t pulse_us){
    ...
    // find hardcorded values for desired initialization
    // printf("CH %i:", ch);
    // printf("slice %i", slice);
    // printf("pwm_chan %i", pwm_chan);
    // printf("level %i\n", level);
    return MBOT_OK;
}
