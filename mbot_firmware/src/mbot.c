...
void register_topics()
{
    ...
    comms_register_topic(MBOT_FORKLIFT_MODE_CMD, sizeof(serial_forklift_mode_t), (Deserialize)&forklift_mode_t_deserialize, (Serialize)&forklift_mode_t_serialize, (MsgCb)mbot_forklift_mode_cmd_cb);
...
void mbot_forklift_mode_cmd_cb(serial_forklift_mode_t *msg)
{
    ...
    if(msg->mode == 1){
        lower_forklift_slowly();
    }
    else if(msg->mode == 2){
        raise_forklift_slowly();
    }
    else if(msg->mode == 3){
        lower_forklift_rapidly();
    }
    else if(msg->mode == 4){
        raise_forklift_rapidly();
    }
    else if(msg-> mode == 5){
        lower_forklift_to_stack();
    }
    else{
        printf("Unknown Forklift Command Was Sent");
    }
}
...
int mbot_init_hardware(void){
    ...
    // init forklift servos
    mbot_servo_init(SV2_CHANNEL);
    mbot_servo_init(SV3_CHANNEL);
...
bool mbot_loop(repeating_timer_t *rt)
{
    ...
    if (global_comms_status == COMMS_OK)
    {
        if(drive_mode == MODE_MOTOR_VEL_OL){
            ...
            printf("Right motor: %f\n", mbot_motor_pwm_cmd.pwm[params.mot_right]);
            printf("Left motor: %f\n", mbot_motor_pwm_cmd.pwm[params.mot_left]);
            if(MBOT_DRIVE_TYPE == OMNI_120_DRIVE){
        ...
        // pass linear and angular velocity commands through low pass filter
        float filtered_linear_vel =  rc_filter_march(&lowpass_linear, mbot_vel_cmd.vx);     // feedforward linear veloicty
        float filtered_angular_vel =  rc_filter_march(&lowpass_angular, mbot_vel_cmd.wz);   // feedforward angular velocity
        // printf("filteredlinear and angular: %f %f\n", filtered_angular_vel, filtered_linear_vel);
        
        // Angular velocity controller 
        // float angular_error = filtered_angular_vel; // feed-forward implmentation, does not account for odometry
        float angular_error = filtered_angular_vel -  mbot_vel.wz;  //feedback with estiamted angular velocity
        // printf("filtered  and mbot_odometry: %f %f \n", filtered_angular_vel ,mbot_vel.wz);
        // printf("mbot.c %f \n", angular_error);
        
        // Run the PID controller for angular velocity
        float pid_output_angular = rc_filter_march(&pid_angular, angular_error);


        mbot_vel_cmd.wz = filtered_angular_vel + pid_output_angular;
        // printf("pid_output_angluar and input : %f %f\n", pid_output_angular, mbot_vel_cmd.wz);


        // Desired angular velocities using kinematics step and linear to angular step
        float v_desired_left = (filtered_linear_vel - (DIFF_BASE_RADIUS * mbot_vel_cmd.wz)) / DIFF_WHEEL_RADIUS;
        float v_desired_right = (-filtered_linear_vel - (DIFF_BASE_RADIUS * mbot_vel_cmd.wz)) / DIFF_WHEEL_RADIUS;
        
        // Measured velocities for left and right wheels from encoders
        float v_measured_left = mbot_motor_vel.velocity[params.mot_left];
        float v_measured_right = mbot_motor_vel.velocity[params.mot_right]; 

        // printf("before PID left : %f %f\n", v_measured_left, v_desired_left);
        // printf("before PID right : %f %f\n", v_measured_right, v_desired_right);

        // Calculate the error for each wheel
        float error_left = v_desired_left - v_measured_left;
        float error_right = v_desired_right - v_measured_right;

        // printf("error left and right : %0.2f %0.2f\n", error_left, error_right);

        // Run the PID controller for left and right wheels
        float pid_output_left = rc_filter_march(&pid_left, error_left);
        float pid_output_right = rc_filter_march(&pid_right, error_right);

        // Feedforward terms (calibrated PWM) for left and right wheels
        float ff_left = _calibrated_pwm_from_vel_cmd(v_desired_left, params.mot_left);
        float ff_right = _calibrated_pwm_from_vel_cmd(v_desired_right, params.mot_right);

        // Combine PID output with feedforward terms to get the final PWM commands
        mbot_motor_pwm_cmd.pwm[params.mot_left] = ff_left + pid_output_left;
        mbot_motor_pwm_cmd.pwm[params.mot_right] = ff_right + pid_output_right;
...
void raise_forklift_slowly(){

    printf("Slowly Raising Lift...\n");
    while(forklift_angle_live_ < forklift_angle_up_){

        sv2_channel_normalized_pulse_ = 0.5 - forklift_angle_live_;
        mbot_servo_send_pulse_normalized(SV2_CHANNEL, sv2_channel_normalized_pulse_);

        sv2_channel_normalized_pulse_ = 0.5 + forklift_angle_live_;
        mbot_servo_send_pulse_normalized(SV3_CHANNEL, sv2_channel_normalized_pulse_);

        forklift_angle_live_ = forklift_angle_live_ + 0.01;
        sleep_ms(75);
    }
    printf("Slowly Raising Lift Complete!\n");
}

void lower_forklift_slowly(){

    printf("Slowly Lowering Lift...\n");
    while(forklift_angle_live_ > forklift_angle_down_){
    ...
    printf("Slowly Lowering Lift Complete!\n");
}

void lower_forklift_rapidly(){

    printf("Rapidly Lowering Lift...\n");
    forklift_angle_live_ = forklift_angle_down_;
    ...
    printf("Rapidly Lowering Lift Complete!\n");
}

void raise_forklift_rapidly(){

    printf("Slowly Raising Lift To Stack...\n");
    while(forklift_angle_live_ < forklift_angle_up_){
    ...
    printf("Slowly Raising Lift To Stack Complete!\n");
}

void lower_forklift_to_stack(){

    printf("Slowly Lowering Lift To Stack...\n");
    while(forklift_angle_live_ > forklift_angle_stack_){
    ...
    printf("Slowly Lowering Lift To Stack Complete!\n");
...
{   
    ...
    lowpass_linear = rc_filter_empty();
    lowpass_angular = rc_filter_empty();
    double dt = 1.0/SAMPLE_RATE;;
    double tc = 0.1;

    rc_filter_first_order_lowpass(&lowpass_linear, dt, tc);
    rc_filter_first_order_lowpass(&lowpass_angular, dt, tc);

    // PID controller angular
    pid_angular = rc_filter_empty();
    double kp_angular = 0.03;    // good for slower speeds 0.4 and 2pi
    // double kp_angular = 0.01;    // good for fast speeds
    double ki_angular = 0.01;    
    double kd_angular = 0.0;   
    double Tf_angular = 1.0/SAMPLE_RATE; // Filter time constant, might need tuning

    // Initialize the PID controller - angular
    rc_filter_pid(&pid_angular, kp_angular, ki_angular, kd_angular, Tf_angular, 1.0/SAMPLE_RATE);

    // PID controller instances for each wheel
    pid_left = rc_filter_empty();
    pid_right = rc_filter_empty();
    double kp_left = 0.08;  
    double ki_left = 0.01;    
    double kd_left = 0.0;   
    double Tf = 1.0/SAMPLE_RATE; 

    double kp_right = 0.02;  
    double ki_right = 0.01;    
    double kd_right = 0.0; 


    // Initialize the PID controller 
    rc_filter_pid(&pid_left, kp_left, ki_left, kd_left, Tf, 1.0/SAMPLE_RATE);
    rc_filter_pid(&pid_right, kp_right, ki_right, kd_right, Tf, 1.0/SAMPLE_RATE);
    ...
    while(running){
        ...
        // Print State
        mbot_print_state(mbot_imu, mbot_encoders, mbot_odometry, mbot_motor_vel);    // print table display
        sleep_ms(200);  
        counter++;
    }
}